...
- “插件”把下载和启用分成两步。下载只写入 Whale 私有缓存；启用时默认打开该插件的全部
  Skills 与 MCP。插件详情提供紧凑的 Skill/MCP/Hook 清单和开关，不提供独立工具页。
- 插件可在约定的 `hooks/hooks.json` 中声明 `Stop` command Hook。Whale 会在启用前展示当前
  平台实际执行的命令并要求明确信任；命令变化后需要重新确认，运行结果以轻量卡片显示在会话中。
